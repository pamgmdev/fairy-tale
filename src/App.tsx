...
function App () 
{
  ...
  const [selectedBook, setSelectedBook] = useState (null); // null = Biblioteca, 'book-1' = Cuento actual
  ...
  const books = [
    {
      ...
      coverModules        : import.meta.glob('./assets/books/book-1/images/covers/*.{png,jpg,jpeg,webp}', { eager: true }),
      chapter0Modules     : import.meta.glob('./assets/books/book-1/images/chapter-0/*.{png,jpg,jpeg,webp}', { eager: true }),
      characterDescModules: import.meta.glob('./assets/books/book-1/images/character-description/*.{png,jpg,jpeg,webp}', { eager: true }),
      chapterModules      : import.meta.glob('./assets/books/book-1/images/chapter-*/*.{png,jpg,jpeg,webp}', { eager: true }),
      finalChapterModules : import.meta.glob('./assets/books/book-1/images/final-chapter/*.{png,jpg,jpeg,webp}', { eager: true }),
      portadaName         : 'cover-1',
  ...
  const portada = selectedBookObj ? Object.keys(selectedBookObj.coverModules).find(p => p.includes(selectedBookObj.portadaName)) : null;
  const contraportada = selectedBookObj ? Object.keys(selectedBookObj.coverModules).find(p => p.includes(selectedBookObj.contraportadaName)) : null;
  ...
  const paginasLibro = useMemo(() => 
  {
    ...
    return [
      ...
      selectedBookObj.coverModules[contraportada]?.default
    ].filter(Boolean);
  ...
  return (
    <div className="main-container">
            ...
            {books.map(book => 
            {
              ...
              return (
                <BookCard
                  ...
                  coverSrc={book.coverModules[portadaKey]?.default}
      ...
      {selectedBook && selectedBookObj && !showBook && (
        <BookPreview
          coverSrc={selectedBookObj.coverModules[portada]?.default}
      ...
      {showBook && isAtLastPage && selectedBookObj && (
        <div className="cover-container">
          <img src={selectedBookObj.coverModules[contraportada]?.default} alt="Contraportada" className="cover-img" />
